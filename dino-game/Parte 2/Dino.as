...
limpa:          JAL     limpatudo             ;se não for o primeiro jogo apaga tudo do terminal
...
loop:           LOAD    R1, M[R4]
                ...
                LOAD    R1, M[R5]
                CMP     R1, R0
                BR.NZ   atualiza             ;se TIMER_TICK diferente de 0 atualiza o jogo e o timer
                BR      skip
                
atualiza:       JAL     PROCESS_TIMER_EVENT
...
WRITE_ON_DISP7: ; SAVE CONTEXT
                ...
                MVI     R5,10
escrevetodos:    ; INSERT VALUE INTO DIGIT Dx
                CMP     R1, R5
                BR.NN   proximodigito
...
sobe:           SUB     R1, R1, R5           ;se estiver a subir muda a posição 
                SUB     R1, R1, R5           ;5 linhas para cima
                SUB     R1, R1, R5
                SUB     R1, R1, R5
                SUB     R1, R1, R5
                
                MVI     R4, EXTREMOS_DINO    ;põe em R4 a str com os extremos do dinossauro   
...
desce:          ADD     R1, R1, R5           ;se estiver a descer muda a posição 
                ADD     R1, R1, R5           ;5 linhas para baixo
                ADD     R1, R1, R5
                ADD     R1, R1, R5
                ADD     R1, R1, R5
...
gameover:       DSI
                MVI     R1,INT_MASK
                MVI     R2,INT_MASK_VAL      ;volta a pôr nas máscara de interrupções o seu valor original  
                STOR    M[R1],R2
                ENI
                ...
                LOAD    R1, M[R1]
                JAL     imprimedino          ;imprime o dinossauro com a pequena animação no olho
                
                JAL     imprimegameover      ;imprime "game over" no terminal

                MVI     R1, PRIMEIRO_JOGO    ;coloca a 1 a variavél PRIMEIRO_JOGO (já não é o primeiro jogo)
                MVI     R2, 1
                STOR    M[R1], R2
                
                MVI     R2,INICIA_JOGO       ;coloca a 0 a variavél INICIA_JOGO  
                STOR    M[R2], R0 

                MVI     R1, OLHO_DINO        ;volta a colcar OLHO_DINO com o valor 0 
                MVI     R2, 0
                STOR    M[R1], R2
                        
                JAL     espera               ;salta para a etiqueta espera que está na função principal  
...
limpaterminal:  MVI     R2, vazia       ;str vazia auxiliar 
                MVI     R4, NEXT_LINE
                DEC     R6
                STOR    M[R6], R1
                JAL     imprimelinha
