...
class RicochetRobots(Problem):
    ...
    def h(self, node: Node):
        ...
        if node.action:
            ...
            if robot == self.board.target[0]:
                return abs(target_pos[0] - robot_pos[0]) + abs(target_pos[1] - robot_pos[1])
            else:
                for target_robot in self.board.robots:
                    if target_robot == self.board.target[0]:
                        target_robot_pos = self.board.robot_position(target_robot)
                        return abs(target_pos[0] - robot_pos[0]) + abs(target_pos[1] - robot_pos[1]) + 1 + abs(target_pos[0] - target_robot_pos[0]) + abs(target_pos[1] - target_robot_pos[1])
        return 1
