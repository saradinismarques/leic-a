...
class Board:
    ...
    #Para verificar estados repetidos
    def same_config(self, board2):
        ocupied2 = []
        tg2 = board2.get_target()
        
        if tg2[0] == 'R':
            ocupied2 += board2.robot_position('G')
            ocupied2 += board2.robot_position('B')
            ocupied2 += board2.robot_position('Y')
        elif tg2[0] == 'G':
            ocupied2 += board2.robot_position('R')
            ocupied2 += board2.robot_position('B')
            ocupied2 += board2.robot_position('Y')
        elif tg2[0] == 'B':
            ocupied2 += board2.robot_position('G')
        ...
        ocupied = []
        if self.target[0] == 'R':
            ocupied += self.board.robot_position('G')
            ocupied += self.board.robot_position('B')
            ocupied += self.board.robot_position('Y')
        elif self.target[0] == 'G':
            ocupied += self.board.robot_position('R')
            ocupied += self.board.robot_position('B')
            ocupied += self.board.robot_position('Y')
        elif self.target[0] == 'B':
            ocupied += self.board.robot_position('G')
            ocupied += self.board.robot_position('R')
            ocupied += self.board.robot_position('Y')
        else:
...
class RicochetRobots(Problem):
    ...
    def h(self, node: Node):
        ...
        # tg_l = int(tg[1])
        # tg_c = int(tg[2])
        # playerBotPosition = board.robot_index_position(tg[0])

        # if not self.goal_test(node.state):
        #     if playerBotPosition[0] == tg_l or playerBotPosition[1] == tg_c:
        #         return 1
        #     else:
        #         return 2
        # else:
        #     return 0
        return h_distance_to_target(node.state.board)
#______________________________________________________________________________________
#Funcoes independentes_________________________________________________________________

def collides(board, bot: str, action):     
    pos = board.robot_index_position(bot)

    if action == "d" and ( pos[0] == board.dimension-1 or board.has_barrier(pos[0], pos[1], 'd') or board.has_barrier(pos[0]+1, pos[1], 'u') or not board.is_empty_cel(pos[0]+1, pos[1]) ):
        return True

    elif action == "u" and (pos[0] == 0 or board.has_barrier(pos[0], pos[1], 'u') or board.has_barrier(pos[0]-1, pos[1], 'd') or not board.is_empty_cel(pos[0]-1, pos[1]) ):
        return True

    elif action == "r" and (pos[1] == board.dimension-1 or board.has_barrier(pos[0], pos[1], 'r') or board.has_barrier(pos[0], pos[1]+1, 'l') or not board.is_empty_cel(pos[0], pos[1]+1) ):
        return True

    elif action == "l" and (pos[1] == 0 or board.has_barrier(pos[0], pos[1], 'l') or board.has_barrier(pos[0], pos[1]-1, 'r') or not board.is_empty_cel(pos[0], pos[1]-1) ):
        return True
    else:
        return False

# devolve as possiveis acoes de um robo
...
#Heuristica: Distancia do robot jogador ao alvo. Distancia de Manhattan
def h_distance_to_target(board):
    target = tuple(board.get_target()) #(R, l, c)
    player_bot = target[0]
    bot_pos = board.robot_position(player_bot)  #(l, c)

    l_bot = int(bot_pos[0])
    c_bot = int(bot_pos[1])

    l_target = int(target[1])
    c_target = int(target[2])

    horizontal_side = abs(l_bot - l_target) 
    vertical_side = abs(c_bot - c_target) 
    manhattan = horizontal_side + vertical_side
    return manhattan
...
if __name__ == "__main__":
    ...
    # ALGS: astar_search, breadth_first_tree_search, 
    # depth_first_tree_search, greedy_search
