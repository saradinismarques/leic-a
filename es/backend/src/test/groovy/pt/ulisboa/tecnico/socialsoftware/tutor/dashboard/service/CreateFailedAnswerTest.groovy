...
class CreateFailedAnswerTest extends FailedAnswersSpockTest {
    ...
    @Unroll
    def "creating a failed answer also creates same question answered=#answered"(){
        given:
        def quiz2 = createQuiz(2)
        def questionAnswer = answerQuiz(answered, false, true, quizQuestion, quiz)
        def questionAnswer1 = answerQuiz(answered, false, true, quizQuestion, quiz2)
        and:
        failedAnswerService.createFailedAnswer(dashboard.getId(), questionAnswer.getId())

        when:
        failedAnswerService.createFailedAnswer(dashboard.getId(), questionAnswer1.getId())

        then:
        def failedAnswer = failedAnswerRepository.findAll().get(0)
        def failedAnswer1 = failedAnswerRepository.findAll().get(1)
