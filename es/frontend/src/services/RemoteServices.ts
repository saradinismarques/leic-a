...
export default class RemoteServices {
    ...
    static async getFailedAnswers(dashboardId: number): Promise<FailedAnswer[]> {
        return httpClient
            .get(`/students/failedanswers/dashboard/${dashboardId}`)
            .then((response) => {
    ...
    static async updateFailedAnswers(dashboardId: number) {
        return httpClient
            .put(`/students/updatefailedanswers/dashboard/${dashboardId}`)
            .catch(async (error) => {
                throw Error(await this.errorMessage(error));
            });
    }
    ...
    static async deleteFailedAnswer(failedAnswerId: number) {
        return httpClient
            .delete(`/students/failedanswers/failedanswer/${failedAnswerId}`)
            .catch(async (error) => {
                throw Error(await this.errorMessage(error));
            });
    }
    ...
    static async getWeeklyScore(dashboardId: number): Promise<WeeklyScore[]> {
      return httpClient
        .get(`/weeklyScores/dashboard/${dashboardId}`)
        .then((response) => {
    ...
    static async updateWeeklyScore(dashboardId: number) {
      return httpClient
        .put(`/students/updateweeklyscore/dashboard/${dashboardId}`)
        .catch(async (error) => {
          throw Error(await this.errorMessage(error));
        });
    }

    static async removeWeeklyScore(weeklyScoreId: number) {
      return httpClient
        .delete(`/students/weeklyScores/weeklyscore/${weeklyScoreId}`)
        .catch(async (error) => {
          throw Error(await this.errorMessage(error));
        });
    }

    // DifficultQuestion Controller

    static async getDifficultQuestions(dashboardId: number): Promise<DifficultQuestion[]> {
        return httpClient
            .get(
                `/difficultQuestions/dashboard/${dashboardId}`
            )
            .then((response) => {
    ...
    static async removeDifficultQuestion(difficultQuestionId: number) {
        return httpClient
            .delete(
                `/difficultQuestions/${difficultQuestionId}`
            )
            .catch(async (error) => {
                throw Error(await this.errorMessage(error));
            });
