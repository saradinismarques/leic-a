...
void receiveMessage(){
    ...
    n_server=recvfrom(fd_server,buffer_server,500,0,(struct sockaddr*)&addr_server,&addrlen_server); 
    if(n_server==-1)/*error*/
    exit(1);
...
void sendStatusReply(char * message){
    puts(message);
    n_server=sendto(fd_server,(void *)message,strlen(message),0,(struct sockaddr*)&addr_server,addrlen_server);
    if(n_server==-1) {
        /*error*/
