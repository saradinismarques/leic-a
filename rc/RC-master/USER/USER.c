...
void parsingSTDinArguments() {
    ...
    if (!strcmp(command, "login")) {
        ...
        puts("Command received in stdin invalid!\n");
        //exit(1);
    }
}
...
int connectToASSocket(){
    ...
    errcode= getaddrinfo(asip,asPORT,&hintsAS,&resAS);
    if(errcode!=0){
        /*error*/
...
int connectToFSSocket() {
    ...
    errcode= getaddrinfo (fsip,fsPORT,&(hintsFS),&(resFS));
    if(errcode!=0){
        /*error*/
...
void readFromSocket(int fd){
    ...
    if(fd == as){
        ...
        if(!strcmp(command, "RRT")){
            ...
            puts(""); //mete nova linha no stdin no fim de receber o ficheiro
            fclose(fp);
